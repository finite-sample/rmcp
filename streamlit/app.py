...
# Page config
st.set_page_config(
    page_title="RMCP - R Econometrics with Claude AI", page_icon="📊", layout="wide"
)
    ...
    if st.button("📊 Generate Economic Panel Data"):
        ...
        for country in countries:
            for year in years:
                ...
                gdp_growth = (
                    base_gdp - 0.3 * (unemployment - 5.5) + np.random.normal(0, 0.5)
                )
        ...
        ts_data = pd.DataFrame(
            {
                ...
                "inflation_rate": 2.5
                + 0.3 * np.sin(2 * np.pi * np.arange(len(dates)) / 365.25)
...
with col3:
    if st.button("🏦 Generate Financial Data"):
        ...
        financial_data["total_loans"] = np.minimum(
            financial_data["total_loans"], financial_data["total_assets"] * 0.8
        )
        financial_data["tier1_capital_ratio"] = np.maximum(
            8, financial_data["tier1_capital_ratio"]
        )
...
if uploaded_file is not None:
    ...
    if st.checkbox("Use generated economic panel data"):
        ...
        st.success(
            f"✅ Using sample data: {data.shape[0]} rows, {data.shape[1]} columns"
        )
    ...
    if st.checkbox("Use generated time series data"):
        ...
        st.success(
            f"✅ Using time series data: {data.shape[0]} rows, {data.shape[1]} columns"
        )
    ...
    if st.checkbox("Use generated financial data"):
        ...
        st.success(
            f"✅ Using financial data: {data.shape[0]} rows, {data.shape[1]} columns"
        )
        ...
        if selected_vars:
            ...
            if len(selected_vars) > 1:
                ...
                chart_type = st.selectbox(
                    "Chart type:", ["Line Chart", "Scatter Plot", "Histogram"]
                )
                ...
                if chart_type == "Line Chart" and len(selected_vars) >= 2:
                    ...
                    y_var = st.selectbox(
                        "Y variable:", [v for v in selected_vars if v != x_var]
                    )
                    if x_var and y_var:
