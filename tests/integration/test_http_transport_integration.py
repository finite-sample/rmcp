...
async def http_server_with_tools():
    ...
    transport = HTTPTransport(host="127.0.0.1", port=0)  # Use port 0 for auto-assignment
    ...
    async def test_tools_list_request(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                await client.post(f"http://127.0.0.1:{transport.port}/", json=initialize_request)
    ...
    async def test_sse_connection(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                async with client.stream("GET", f"http://127.0.0.1:{transport.port}/sse") as response:
                    assert response.status_code == 200
                        ...
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                if parsed_data.get("method") == "notification/test":
                                    ...
                                    assert parsed_data["params"]["message"] == "test notification"
                                    break
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"http://127.0.0.1:{transport.port}/health",
                    timeout=5.0
                )
