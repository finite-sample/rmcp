...
class MCPProtocolTester:
    ...
    async def send_request(self, method: str, params: dict = None, request_id: int = None):
    ...
    def validate_response_structure(self, response: dict, request_id: int = None):
        ...
        if request_id is not None:
            ...
            assert response["id"] == request_id, f"Response id mismatch: {response['id']} != {request_id}"
...
async def test_mcp_tool_execution(mcp_tester):
    ...
    json_content = None
    for item in content:
        if item.get("type") == "text" and "application/json" in item.get("annotations", {}).get("mimeType", ""):
...
async def test_complete_mcp_conversation():
    ...
    # 1. Initialize
    init_response = await tester.send_request("initialize", {
        "protocolVersion": "2025-06-18",
        "capabilities": {"tools": {}},
    ...
    if tools:
        ...
        if tool_name == "linear_model":
            tool_response = await tester.send_request("tools/call", {
                "name": tool_name,
                "arguments": {
...
def test_mcp_protocol_version_compatibility():
    ...
    assert expected_version.count("-") == 2, "Protocol version should be YYYY-MM-DD format"
