"""
Integration tests for new features in v0.3.5.
...
async def create_integration_server():
    ...
    register_tool_functions(
        ...
        # Existing tools
        linear_model, correlation_analysis
    )
...
async def test_formula_to_analysis_workflow():
    ...
    print(f"   ✅ Analysis completed: {len(analysis_result.get('correlation_matrix', {}))} correlations")
...
async def test_data_validation_integration():
    ...
    for dataset_name, analysis_type in zip(datasets, analysis_types):
        ...
        validation_result = json.loads(response['result']['content'][0]['text'])
        print(f"   ✅ {dataset_name} validated for {analysis_type}: {'✓' if validation_result['is_valid'] else '⚠'}")
