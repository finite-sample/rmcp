...
class TestMCPErrorProtocolCompliance:
    ...
    def validate_jsonrpc_error_structure(self, response: Dict[str, Any], request_id: Any = None):
    ...
    def validate_mcp_tool_error_structure(self, response: Dict[str, Any]):
        ...
        assert first_content["type"] in ["text", "resource"], "Content type must be text or resource"
    ...
    async def test_unknown_tool_error_protocol(self):
        ...
        assert "unknown tool" in error["message"].lower(), "Message should mention unknown tool"
    ...
    async def test_r_execution_error_protocol(self):
        ...
        content_text = response["result"]["content"][0]["text"]
        assert "tool execution error" in content_text.lower() or "error" in content_text.lower()
    ...
    async def test_file_error_protocol(self):
        ...
        content_text = response["result"]["content"][0]["text"]
        assert any(keyword in content_text.lower() for keyword in ["file", "not found", "does not exist"])
    ...
    async def test_error_message_localization_ready(self):
        ...
        for case in test_cases:
            ...
            assert message[0].isupper(), "Error messages should start with capital letter"
    ...
    async def test_error_response_timing(self):
        ...
        assert response_time < 1.0, f"Error response took {response_time:.2f}s (should be < 1s)"
    ...
    async def test_error_content_encoding(self):
        ...
        assert "café" not in response_json or len(response_json) > 0  # Either filtered or encoded
...
class TestMCPErrorMetadata:
    ...
    async def test_error_metadata_structure(self):
        ...
        if "result" in response and response["result"].get("isError"):
            ...
            helpful_keywords = ["data", "observations", "sample", "size", "degrees", "freedom"]
        ...
        for i, test_case in enumerate(error_categories):
            ...
            if "result" in response and response["result"].get("isError"):
                ...
                if test_case["expected_category"] == "validation_error":
                    ...
                    assert any(keyword in text.lower() for keyword in ["file", "not found", "does not exist"])
            
            print(f"✅ Error categorization verified for {test_case['expected_category']}")
