...
class TestClaudeErrorExperienceScenarios:
    ...
    async def test_novice_user_data_error_experience(self, claude_server, context):
        ...
        data_issue_indicators = ["data", "mismatch", "length", "type", "numeric", "character"]
        ...
        helpful_actions = ["check", "ensure", "make sure", "verify", "try", "use", "convert"]
    ...
    @pytest.mark.asyncio
    async def test_researcher_statistical_warning_experience(self, claude_server, context):
        ...
        if result["success"]:
            ...
            if validation_result["success"]:
                ...
                if warnings:
                    ...
                    statistical_concepts = ["separation", "convergence", "sample", "assumption"]
                    assert any(concept in warnings_text for concept in statistical_concepts), \
            ...
            error_lower = error_msg.lower()
            statistical_terms = ["convergence", "separation", "optimization", "fitted", "probabilities"]
        ...
        if fix_result["success"]:
            ...
            suggestions_text = " ".join(suggestions).lower()
            statistical_solutions = ["regularization", "penalized", "sample", "data", "transform", "interaction"]
            assert any(solution in suggestions_text for solution in statistical_solutions), \
    ...
    async def test_business_analyst_file_error_experience(self, claude_server, context):
        ...
        error_lower = error_msg.lower()
        file_indicators = ["file", "not found", "does not exist", "path", "cannot access"]
        ...
        assert "/wrong/path/sales_data.csv" in error_msg or "sales_data.csv" in error_msg, \
        ...
        suggestions_text = " ".join(suggestions).lower()
        file_help = ["check path", "verify", "file exists", "permissions", "directory", "absolute path"]
    ...
    async def test_data_scientist_ml_error_experience(self, claude_server, context):
        ...
        if not result["success"]:
            ...
            error_lower = error_msg.lower()
            ml_concepts = ["classification", "tree", "split", "variance", "target", "feature"]
        ...
        if not validation_data.get("is_valid", True):
            ...
            ml_issues = ["variation", "constant", "split", "classification", "target", "feature"]
    ...
    async def test_progressive_error_recovery_workflow(self, claude_server, context):
        ...
        step1 = await simulate_claude_analysis(
            ...
            {
                "data": initial_data,
                "formula": "revenue ~ quarter"
            },
            "Analyze revenue trends by quarter"
        )
        
        recovery_steps.append(("Initial analysis", step1["success"], step1.get("error", "")))
        ...
        if not step1["success"]:
            ...
            if step2["success"]:
                ...
                if step3["success"]:
                    ...
                    suggestions_text = " ".join(suggestions).lower()
                    progressive_actions = ["first", "then", "next", "after", "before", "ensure"]
                    logical_flow = any(action in suggestions_text for action in progressive_actions)
                    
                    # Should address specific issues found
                    addresses_length = "length" in suggestions_text or "mismatch" in suggestions_text
                    addresses_missing = "missing" in suggestions_text or "na" in suggestions_text
        ...
        assert successful_steps >= 2, f"At least 2 recovery steps should succeed: {recovery_steps}"
...
class TestErrorMessageQualityForClaude:
    ...
    async def test_error_messages_conversational_tone(self, claude_server, context):
        ...
        for scenario in scenarios:
            ...
            if not result["success"]:
                ...
                assert not error_msg.startswith("Error:"), "Should not start with 'Error:'"
                assert len(error_msg.split()) > 3, "Should be more than just error code"
                
                # Should provide context
                error_lower = error_msg.lower()
                context_indicators = ["analysis", "data", "file", "statistical", "model"]
                assert any(indicator in error_lower for indicator in context_indicators), \
    ...
    async def test_error_message_actionability(self, claude_server, context):
        ...
        if not result["success"]:
            ...
            error_lower = error_msg.lower()
            actionable_words = ["try", "ensure", "check", "add", "provide", "use", "consider"]
