...
class IDEConfigValidator:
            ...
            if config_path.exists():
                try:
                    ...
                    for server_name, server_config in mcp_servers.items():
                        if ("rmcp" in server_name.lower() or 
                            server_config.get("command") == "rmcp"):
                    ...
                    if rmcp_config.get("command") != "rmcp":
                        return False, f"Invalid command in RMCP config: {rmcp_config.get('command')}", config
                    
                    if "start" not in rmcp_config.get("args", []):
                        return False, "RMCP config missing 'start' argument", config
                    
                    return True, f"RMCP properly configured as '{rmcp_name}' in {config_path}", config
    ...
    def validate_vscode(self) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            if "continue.continue" not in extensions:
                return False, "Continue extension not installed. Run: code --install-extension Continue.continue", {}
        except Exception as e:
    ...
    def test_rmcp_server(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return False, f"No valid JSON response from RMCP server. stdout: {stdout}, stderr: {stderr}"
...
def main():
    ...
    parser.add_argument("--ide", choices=["claude", "vscode", "cursor"], 
                       help="Validate specific IDE only")
    parser.add_argument("--create-samples", action="store_true",
                       help="Create sample configuration files")
    parser.add_argument("--setup-instructions", metavar="IDE",
                       help="Show setup instructions for IDE")
