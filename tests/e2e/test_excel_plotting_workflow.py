...
async def simulate_claude_desktop_workflow():
    ...
    read_request = {
        ...
        "params": {
            "name": "read_excel",
            "arguments": {"file_path": excel_path}
        }
    }
    ...
    try:
        ...
        if "result" in response and "content" in response["result"]:
            ...
            print(f"   📊 Data shape: {file_info.get('n_rows', 'unknown')} rows, {file_info.get('n_cols', 'unknown')} columns")
...
async def test_other_problematic_tools():
    ...
    register_tool_functions(
        server.tools,
        read_csv, data_info, linear_model, correlation_analysis,
        summary_stats, t_test
    )
...
async def main():
    ...
    print("=" * 60)
    print(f"Excel + Scatter Plot Workflow: {'✅ PASSED' if workflow_success else '❌ FAILED'}")
