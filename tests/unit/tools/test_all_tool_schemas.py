...
def get_test_tools() -> List[Tuple[str, Any, Dict[str, Any]]]:
    ...
    tools_test_data = [
        # Regression tools
        ("linear_model", regression.linear_model, {
            "data": {
...
class TestAllToolSchemas:
    ...
    def test_tool_input_schema_validation(self, tool_name: str, tool_func: Any, test_input: Dict[str, Any]):
        """Test that all tools have valid input schemas and accept realistic data."""
        # Verify tool has schema
        assert hasattr(tool_func, '_mcp_tool_input_schema'), f"{tool_name} missing input schema"
        ...
        try:
            ...
            pytest.fail(f"{tool_name} schema validation failed with realistic data: {e}")

    @pytest.mark.parametrize("tool_name,tool_func,test_input", get_test_tools())
    def test_tool_function_signature(self, tool_name: str, tool_func: Any, test_input: Dict[str, Any]):
    ...
    @pytest.mark.parametrize("tool_name,tool_func,test_input", get_test_tools())
    def test_tool_metadata_completeness(self, tool_name: str, tool_func: Any, test_input: Dict[str, Any]):
        """Test that all tools have complete metadata."""
        # Check for required MCP tool attributes
        assert hasattr(tool_func, '_mcp_tool_name'), f"{tool_name} missing _mcp_tool_name"
        assert hasattr(tool_func, '_mcp_tool_description'), f"{tool_name} missing _mcp_tool_description"
        assert hasattr(tool_func, '_mcp_tool_input_schema'), f"{tool_name} missing _mcp_tool_input_schema"
        
        # Verify metadata values
        assert tool_func._mcp_tool_name == tool_name, f"{tool_name} name mismatch"
        assert isinstance(tool_func._mcp_tool_description, str), f"{tool_name} description not string"
        assert len(tool_func._mcp_tool_description) > 0, f"{tool_name} empty description"
...
class TestToolSchemaStructure:
    ...
    @pytest.mark.parametrize("tool_name,tool_func,test_input", get_test_tools())
    def test_data_parameter_structure(self, tool_name: str, tool_func: Any, test_input: Dict[str, Any]):
        """Test that tools with data parameters have consistent structure."""
        schema = tool_func._mcp_tool_input_schema
        
        if "data" in schema.get("properties", {}):
            data_schema = schema["properties"]["data"]
            assert data_schema["type"] == "object", f"{tool_name} data parameter should be object type"
            assert "properties" in data_schema, f"{tool_name} data parameter missing properties"

    @pytest.mark.parametrize("tool_name,tool_func,test_input", get_test_tools())
    def test_required_fields_present(self, tool_name: str, tool_func: Any, test_input: Dict[str, Any]):
        """Test that required fields are properly specified."""
        schema = tool_func._mcp_tool_input_schema
        
        if "required" in schema:
            required_fields = schema["required"]
            assert isinstance(required_fields, list), f"{tool_name} required should be list"
            
            # Check that all required fields exist in properties
            properties = schema.get("properties", {})
            for field in required_fields:
                assert field in properties, f"{tool_name} required field '{field}' not in properties"

    @pytest.mark.parametrize("tool_name,tool_func,test_input", get_test_tools())
    def test_enum_values_validation(self, tool_name: str, tool_func: Any, test_input: Dict[str, Any]):
        ...
        def check_enum_in_schema(obj):
            if isinstance(obj, dict):
                if "enum" in obj:
                    ...
                    assert isinstance(enum_values, list), f"{tool_name} enum should be list"
                    assert len(enum_values) > 0, f"{tool_name} enum should not be empty"
                    assert len(set(enum_values)) == len(enum_values), f"{tool_name} enum has duplicates"
...
class TestToolSchemaEdgeCases:
    ...
    def test_invalid_input_rejection(self):
        ...
        invalid_inputs = [
            ...
            {"data": {"x": [1, 2]}, "variable": "x", "alternative": "invalid_option"},  # Invalid enum
        ]
