...
@tool(
    ...
    description="Creates lagged (past) and lead (future) variables for time series or panel data.",
)
...
@tool(
    ...
    description="Caps extreme values at given percentiles instead of dropping them. Retains all observations, unlike trimming.",
)
...
@tool(
    ...
    description="First, seasonal, or higher-order differencing to make a series stationary. Standard preprocessing before ARIMA.",
)
...
@tool(
    ...
    description="Rescales variables by z-score, min-max, or robust (median/MAD) scaling. Returns the scaling parameters needed to invert the transform.",
)
